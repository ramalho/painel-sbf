...
// jumper interno para manutencao
// LOW: habilita painel
const int pino_geral = 12;

// sinal de curto na linha
// LOW: indica curto
const int pino_curto = 11;

const int botao_habilita = A1;  // botao que habilita linha`
const int led_habilita = 13;    // led do botao que habilita linha
const int rele_habilita = 10;   // rele que habilita a linha

const int botao_externo = 5;    // botao que seleciona fonte externa
const int led_externo = A4;     // led do botao que seleciona fonte externa
const int rele_externo = 9;     // rele que seleciona fonte externa

const int botao_digital = 3;
const int led_digital = 2;
const int rele_digital = 7;

const int botao_analogico = 4;
const int led_analogico = A5;
const int rele_analogico = 8;

const int botao_wifi = A3;
const int led_wifi = A0;
const int rele_wifi = 6;
...
void loop() {
  int leitura_geral = digitalRead(pino_geral); 
  if (leitura_geral != 0) {
    digitalWrite(rele_habilita, HIGH);
    digitalWrite(led_habilita, HIGH);
    digitalWrite(rele_externo, HIGH);
...
void curto() {
  ...
  if (leitura_curto != 1) {
    ...
    static boolean estado_rele_habilita = false;  //Armazena o estado do relé (ligado/desligado)
